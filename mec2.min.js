 ...
 * @requires g2.js
 */
"use strict";mec.view={extend(t){if(t.type&&mec.view[t.type]){Object.setPrototypeOf(t,mec.view[t.type]);t.constructor()}return t}};mec.view.vector={constructor(){},init(t){if(typeof this.p==="string")this.p=t.nodeById(this.p)},dependsOn(t){return this.p===t},asJSON(){return'{ "type":"'+this.type+',"id":'+this.id+',"p":'+this.p.id+(this.value?',"value":'+this.value:"")+" }"},get isSolid(){return false},get sh(){return this.state&g2.OVER?[0,0,10,mec.hoveredElmColor]:false},get endPoints(){const t=mec.aly[this.value].scl;const i=this.p[this.value]();const s=Math.hypot(i.y,i.x);const e=!mec.isEps(s)?mec.asympClamp(t*s,25,150):0;return{p1:this.p,p2:{x:this.p.x+i.x/s*e,y:this.p.y+i.y/s*e}}},hitContour({x:t,y:i,eps:s}){const e=this.endPoints;return g2.isPntOnLin({x:t,y:i},e.p1,e.p2,s)},g2(){const t=this.endPoints;return g2().vec({x1:t.p1.x,y1:t.p1.y,x2:t.p2.x,y2:t.p2.y,ls:mec.color[this.value],lw:1.5,sh:this.sh})}};
/**
 * mec.shape (c) 2018 Stefan Goessner
 * @license MIT License
 * @requires mec.core.js
